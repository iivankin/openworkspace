...
9. **Show original authentication** — Optional. Create a Cloudflare API token with **Analytics Read** for the mail zone, then set `CLOUDFLARE_ZONE_ID` and `CLOUDFLARE_ANALYTICS_TOKEN` with `wrangler secret put`. Show original will display Cloudflare's SPF, DKIM, DMARC, and spam results for incoming messages. Analytics can only be fetched during Cloudflare's 31-day retention window; fetched results are retained with the message.
